...
class UpdateProfileTests(TestCase):
    ...
    def test_invalid_user(self):
        ...
        response = self.client.post(get_path(str(deletedId)), data=body, content_type="application/json")
        self.assertEqual(response.status_code, 404)

    def test_invalid_github(self):
        body = get_request_body("abc", "a", "b", "x@y.com", "https://gitgud.com/username", "abc")
        response = self.client.post(get_path(str(self.author1.id)), data=body, content_type="application/json")
